...
class AuthGroup extends Base {
    ...
    public function addToGroup($uid,$gid){
	    ...
	    $add = array();
          foreach ($uid_arr as $u){
            	...
            	//判断用户id是否合法
            	if(Db::name('Member')->getFieldByUid($u,'uid') == false){
    ...
    static public function getExtendOfGroup($gid,$type){
        ...
        return Db::name(self::auth_extend)->where( array('group_id'=>$gid,'type'=>$type) )->column('extend_id');
    }
    ...
    public function checkId($modelname,$mid,$msg = '以下id不存在:'){
        ...
        $s = Db::name($modelname)->where(array('id'=>array('IN',$ids)))->column('id');
