                ...
                <li class="pull-right">
                    <select class="form-control" name="group">
                        <?php if(is_array($auth_group) || $auth_group instanceof \think\Collection || $auth_group instanceof \think\Paginator): $i = 0; $__LIST__ = $auth_group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                            <option value="<?php echo url('AuthManager/category',array('group_id'=>$vo['id'],'group_name'=>$vo['title'])); ?>" <?php if($vo['checked'] == 'true'): ?>selected<?php endif; ?> ><?php echo $vo['title']; ?></option>
                        <?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </li>
            </ul>
...
<script type="text/javascript" >
    ...
    var zNodes =<?php echo json_encode($category_list); ?>;
    ...
    $(document).ready(function(){
        ...
        $(".ajax-posts").click(function(){
            ...
            $.post("<?php echo url('AuthManager/addToCategory'); ?>",{'cid':data,'group_id':group_id}).done(function(data){
                if(data.code == 1){
