<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:73:"D:\www\TwoThink\public/../application/admin/view/default/active\edit.html";i:1522036463;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
...
<div class="main">
    ...
    <div class="container-fluid">
        ...
        <form action=" <?php echo url('admin/active/edit'); ?>" method="post">
            <div class="form-group">
                <label>标题(必填):</label>
                <input type="text"  name="title" value="<?=$services['title']?>" class="form-control" />
                <input type="hidden"  name="id" value="<?=$services['id']?>" class="form-control" />
            </div>
            <div class="form-group">
                <label>内容(必填):</label>
            ...
            <div class="form-group">
                ...
                发布:<input type="radio" name="status"   value="1" <?=$services['status']==1?'checked':''?> />
                禁用:<input type="radio" name="status"   value="2" <?=$services['status']==1?'':'checked'?>/>
            </div>
            <div class="form-group">
                <label>活动时间(必填):</label><br>
                开始时间:<input type="date" name="start_time"   value="<?=date('Y-m-d',$services['start_time'])?>"/>
                结束时间:<input type="date" name="end_time"   value="<?=date('Y-m-d',$services['end_time'])?>"/>
            </div>
            <div class="form-group">
                <label>发布者(必填):</label>
                <input type="text" class="form-control" name="author" value="<?=$services['author']?>"/>
            </div>
            <div class="form-group">
                <label>图片(小区配图):</label>
                ...
                <img id="logo_view" width="150px" src="<?=$services['picture']?>">
